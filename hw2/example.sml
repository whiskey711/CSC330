...
fun similar_names (lst, full_name) =
  let fun construct_names(lst, full_name, accumulator) =
        ...
        | x::xs => case full_name of
                     {first, middle, last} => construct_names(xs, full_name, {first=x, middle=middle, last=last} :: accumulator)
  in
    case full_name of
      {first, middle, last} => construct_names(first::get_substitutions2(lst, first), full_name, [])
  end
